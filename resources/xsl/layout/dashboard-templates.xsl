...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="published_documents">
    ...
    <xsl:variable name="headline">
      <xsl:choose>
        <xsl:when test="mcrxsl:isCurrentUserInRole('editor') or mcrxsl:isCurrentUserInRole('admin')">
          <xsl:copy-of select="i18n:translate('dashboard.header.admin.published_documents" /> 
        </xsl:when>
        <xsl:otherwise>
          <xsl:copy-of select="i18n:translate('dashboard.header.user.published_documents" /> 
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
  ...
  <xsl:template match="unpublished_documents">
    ...
    <xsl:variable name="headline">
      <xsl:choose>
        <xsl:when test="mcrxsl:isCurrentUserInRole('editor') or mcrxsl:isCurrentUserInRole('admin')">
          <xsl:copy-of select="i18n:translate('dashboard.header.admin.unpublished_documents"/> 
        </xsl:when>
        <xsl:otherwise>
          <xsl:copy-of select="i18n:translate('dashboard.header.user.unpublished_documents" /> 
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
